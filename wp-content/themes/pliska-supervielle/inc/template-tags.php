...
if ( ! function_exists( 'pliska_updated_on' ) ) :
	function pliska_updated_on() {
		...
		if ( $show_modified_date || is_customize_preview() ) {
			...
			$updated_on  = sprintf(
				esc_html_x( 'Mis à jour %s', 'post date', 'pliska' ),
				'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
			);
...
function pliska_blog_read_time() {
	...
	if ( $show_time_to_read || is_customize_preview() ) {
		...
		printf( '<span class="screen-reader-text">' . __( 'Post read time', 'pliska' ) . '</span><span class="time-read-links' . pliska_add_customizer_class( $show_time_to_read ) . '">' . wp_kses( pliska_get_svg( 'clock' ), pliska_get_kses_extended_ruleset() ) .  '</span>' );
	}
